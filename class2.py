...
def training(class1,class2, Obs, list_best_accuracy):
	print("\n" + "+"*40)
	print("+++++ Training for %s vs. %s classifier" % (class1,class2))
	label_plot= ("%s_Obs_%s_vs_%s.png" % (Obs,class1,class2))
	print("+"*40 + "\n")
	...
	for numIter,stepSize,regParam in itertools.product(numIters,stepSizes,regParams):
		...
		if class2 == "All":
			...
			model_acc2.append(accuracy_All)
	...
	list_best_accuracy.append(best_accuracy)

	plt.plot(numIters, model_acc, label='vs 1' )
	if class2 == "All": plt.plot(numIters, model_acc2, label='vs All' )
	plt.ylabel('precision')
	plt.xlabel('numbrs of iter ')
	plt.title(label_plot)
	plt.legend()
	plt.savefig("/home/maria_dev/openclassroom/"+ label_plot)
	plt.gcf().clear()
	plt.clf()
	plt.cla()
	plt.close()

	return list_best_accuracy
...
def main():
	...
	features_value_list2 = load_features_value(features_dir_all)
	...
	for class1 in classes:
		...
		for class2 in class2_set:
			...
			for Obs in Observations:
				train_features_df, test_features_df = features_classifier_df.randomSplit([Obs, 1 -Obs])
				...
				list_best_accuracy = training(class1,class2, Obs, list_best_accuracy)
	plt.plot(Observations, list_best_accuracy )
	plt.ylabel('precision')
	plt.xlabel('numbrs of Observations ')
	plt.title("1vs1 classifiers")
	plt.legend()
	plt.savefig("/home/maria_dev/openclassroom/1vs1.png")
	plt.gcf().clear()
	plt.clf()
	plt.cla()
	plt.close()
	del list_best_accuracy[:]
	...
	for class1 in classes:
		...
		print("_____ Spliting data into training & test data..")
		for Obs in Observations:
			train_features_df, test_features_df = features_df.randomSplit([Obs, 1 -Obs])
			train_features_df2, test_features_df2 = features_df2.randomSplit([Obs, 1 -Obs])
			...
			print("%i training data" % (train_count))
			print("%i testing data" % (test_count))
			print("%i testing All data" % (test_count2))
			# [CONVERET LABELS] Convert string labels into floats with an estimator
			print("_____ Converting string labels into floats with an estimator..")
			...
			# [BUILD MODEL] Learn classifier on training data
			print(">>>>> Training classifier..")
			list_best_accuracy= training(class1,"All", Obs, list_best_accuracy)
		plt.plot(Observations, list_best_accuracy )
		plt.ylabel('precision')
		plt.xlabel('numbrs of Observations ')
		plt.title("1vsALL classifiers")
		plt.legend()
		plt.savefig("/home/maria_dev/openclassroom/"+class1+"_1vsALL.png")
		plt.gcf().clear()
		plt.clf()
		plt.cla()
		plt.close()
